...
function GeometryLoader(bimServerApi, models, viewer) {
	...
	this.readStart = function(data){
		...
		if (firstModel) {
            ...
            //
			// lookat.set("eye", eye);
             eyeY=modelBounds.max.y * -2;
			 eyeZ=(modelBounds.max.z - modelBounds.min.z)* 0.5;
			lookat.set({
				//eye: { x: 0,y: modelBounds.max.y * -2, z: (modelBounds.max.z - modelBounds.min.z)  },
			...
			/*var rotate=o.viewer.scene.findNode("myRotate");
			rotate.set({
				y:0,
				x:0,
				z:10,
			...
			mfar=diagonal * 5;
			maspect=jQuery(o.viewer.canvas).width() / jQuery(o.viewer.canvas).height();
