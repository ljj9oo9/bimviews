...
BIMSURFER.Control.PickFlyOrbit = BIMSURFER.Class(BIMSURFER.Control, {
	...
	initEvents: function() {
		...
		if(this.active) {
			...
			this.SYSTEM.events.register('keydown', this.keydown, this);
			this.SYSTEM.events.register('pick', this.pick, this);
			this.SYSTEM.events.register('tick', this.tick, this);


			document.addEventListener('roatess', this.roatess,true);
			...
			this.SYSTEM.events.unregister('keydown',this.keydown,this);
			this.SYSTEM.events.unregister('pick', this.pick, this);
			this.SYSTEM.events.unregister('tick', this.tick, this);
			document.removeEventListener('keydown', this.keydown,true);
			document.removeEventListener('roatess', this.roatess,true);
		}
		return this;
	},
	...
	keydown: function(e) {
		...
		if(e.altKey && keyID === 38)  { // alt+up  向上平移
			lookZ-=1;
			eyeZ-=1;
		...
		if(e.altKey &&keyID ===40 )  { // alt+down  向下平移
			lookZ+=1;
			eyeZ+=1;
			lookatAll.set({
				eye: { x: eyeX, y: eyeY, z:eyeZ   },
		...
		if(e.altKey &&keyID === 37)  { // alt+left  向左平移
			lookX+=1;
			eyeX+=1;
			lookatAll.set({
				eye: { x: eyeX, y: eyeY, z:eyeZ  },
		...
		if(e.altKey &&keyID == 39 )  { // alt+right  向右平移
		...
		if(e.shiftKey && keyID === 38)  { // shift+UP     缩小效果
		...
		if(e.shiftKey &&keyID === 40 )  { // shift+down 放大效果
			//lookY-=1;
			// eyeY-=1;
			lookatAll.set({
				eye: { x: eyeX, y: eyeY, z:eyeZ   },
		...
		if(e.ctrlKey &&keyID === 38 )  { // ctrl+UP 向上翻滚  以X为轴
			angleZ-=1;
			RotateRowX=1;
			RotateRowZ=0;
		   RollingAround.set({
		...
		if(e.ctrlKey &&keyID === 40 )  { // ctrl+down 向下翻滚  以X为轴
			angleZ+=1;
		...
		if(e.ctrlKey &&keyID === 37 )  { // ctrl+left 向左翻滚  以Z为轴
		...
		if(e.ctrlKey &&keyID === 39 )  { //  ctrl+right 向右翻滚  以Z为轴
			angleX+=1;
